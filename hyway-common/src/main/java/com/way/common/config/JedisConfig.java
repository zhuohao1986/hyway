 ...
 * org.springframework.cache.interceptor.CacheErrorHandler; import
 ...
 * 
 * import com.fasterxml.jackson.annotation.PropertyAccessor; import
 * com.fasterxml.jackson.databind.ObjectMapper;
 * 
 * import redis.clients.jedis.JedisPool; import
 * redis.clients.jedis.JedisPoolConfig;
 * 
 * @Configuration public class JedisConfig extends CachingConfigurerSupport {
 * 
 * private Logger logger = LoggerFactory.getLogger(JedisConfig.class);
 * 
 * @Value("${spring.redis.database}") private String database;
 * 
 ...
 * @Bean
 * 
 * @Override public KeyGenerator keyGenerator() { // 设置自动key的生成规则，配置spring
 * boot的注解，进行方法级别的缓存 // 使用：进行分割，可以很多显示出层级关系 //
 * 这里其实就是new了一个KeyGenerator对象，只是这是lambda表达式的写法，我感觉很好用，大家感兴趣可以去了解下、 return
 * (target, method, params) -> { StringBuilder sb = new StringBuilder();
 * sb.append(target.getClass().getName()); sb.append(":");
 * sb.append(method.getName()); for (Object obj : params) { sb.append(":" +
 * String.valueOf(obj)); } String rsToUse = String.valueOf(sb);
 * logger.info("自动生成Redis Key -> [{}]", rsToUse); return rsToUse; }; }
 * 
 * @Bean
 * 
 * @Override public CacheManager cacheManager() { //
 * 初始化缓存管理器，在这里我们可以缓存的整体过期时间什么的，我这里默认没有配置 logger.info("初始化 -> [{}]",
 ...
 * redisTemplate(JedisConnectionFactory jedisConnectionFactory ) { //设置序列化
 ...
 * Object>(); redisTemplate.setConnectionFactory(jedisConnectionFactory);
 ...
 * @Override
 * 
 * @Bean public CacheErrorHandler errorHandler() { //
 * 异常处理，当Redis发生异常时，打印日志，但是程序正常走 logger.info("初始化 -> [{}]",
 * "Redis CacheErrorHandler"); CacheErrorHandler cacheErrorHandler = new
 * CacheErrorHandler() {
 * 
 * @Override public void handleCacheGetError(RuntimeException e, Cache cache,
 * Object key) { logger.error("Redis occur handleCacheGetError：key -> [{}]",
 * key, e); }
 * 
 * @Override public void handleCachePutError(RuntimeException e, Cache cache,
 * Object key, Object value) {
 * logger.error("Redis occur handleCachePutError：key -> [{}]；value -> [{}]",
 * key, value, e); }
 * 
 * @Override public void handleCacheEvictError(RuntimeException e, Cache cache,
 * Object key) {
 ...
 * { logger.error("Redis occur handleCacheClearError：", e); } }; return
 * cacheErrorHandler; }
 * 
 ...
 * jedisPoolConfig.setMaxTotal(maxActive); jedisPoolConfig.setMinIdle(minIdle);
 * JedisPool jedisPool = new JedisPool(jedisPoolConfig, host, port, timeout,
 * password); logger.info("JedisPool注入成功！"); logger.info("redis地址：" + host + ":"
