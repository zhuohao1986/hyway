...
import com.way.im.net.server.ServerListen;
...
public class ClientActivity {
	...
	public void checkAccount(String account) {
		...
		boolean isExisted = userService.selectAccount(account);
	...
	public void login(TranObject tran) {
		...
		boolean isExisted = userService.login(user);
		if (isExisted == true) {
			userService.updateIsOnline(user.getId(), 1);
			setUser(user);
			...
			// 获取好友列表
			List<User> friendList = friendService.getFriend(user.getId());
		...
		// 获取离线信息
		List<TranObject> offMsg = msgService.selectMsg(user.getId());
		for (int i = 0; i < offMsg.size(); i++)
			insertQueue(offMsg.get(i));
		msgService.deleteSaveMsg(user.getId());
	...
	public void regist(TranObject tran) {
		User user = (User) tran.getObject();
		int id = userService.insertInfo(user);
		user.setId(id);
		if (id == -1)
	...
	public void getOffLine() {
		...
		userService.updateIsOnline(user.getId(), 0);
	}
	...
	public void searchFriend(TranObject tran) {
		...
		if (values[0].equals("0"))
			list = userService.selectFriendByAccountOrID(values[1]);
		else
			list = userService.selectFriendByMix(values);
	...
	public void friendRequset(TranObject tran) {
		...
		if (result == Result.FRIEND_REQUEST_RESPONSE_ACCEPT) {
			...
			friendService.addFriend(tran.getReceiveId(), tran.getSendId());
			friendService.addFriend(tran.getSendId(), tran.getReceiveId());
			System.out.println("添加好友成功....");
			// 向好友发起方 发送自己的信息
			tran.setObject(user);
			List<User> friend = userService.selectFriendByAccountOrID(tran
					.getSendId());
			tran.setObject(friend.get(0));
			tran.setSendName(user.getUserName());
			// 向自己添加好友
			friend = userService.selectFriendByAccountOrID(tran.getReceiveId());
	...
	public void sendFriend(TranObject tran) {
		...
		if (mServer.contatinId(tran.getReceiveId())) {
			...
			msgService.insertSaveMsg(user.getId(), tran);
		}
