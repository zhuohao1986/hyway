...
public class JwtBuilderUtil {
	...
	private static final Logger log = LoggerFactory.getLogger(JwtBuilderUtil.class);
	/**
	 * JWT 加解密类型
	 */
	 ...
	 * JWT 生成密钥使用的密码
	 */
	private static final String JWT_RULE = "hyway.live";
	...
	public static SecretKey generateKey() {
		SecretKey secretKey = generateKey(JWT_ALG, JWT_RULE);
		return secretKey;
	}
	...
	public static String buildJWT(SignatureAlgorithm alg, Key key, String sub, String aud, String jti, String iss,
		...
		Date exp = null;
		if (duration != null)
			exp = (nbf == null ? plusdate : nbfdate);

		// 获取JWT字符串
		String compact = Jwts.builder().signWith(alg, key).setSubject(sub).setAudience(aud).setId(jti).setIssuer(iss)
				.setNotBefore(nbf).setIssuedAt(date).setExpiration(exp != null ? exp : null).compact();
